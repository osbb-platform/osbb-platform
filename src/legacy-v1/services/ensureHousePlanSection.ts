...
export async function ensureHousePlanSection(params: {
  housePageId: string;
}) {
  ...
  if (existingError) {
    throw new Error(
      `Failed to check plan section: ${existingError.message}`,
    );
  }
  ...
  if (createError || !createdSection?.id) {
    throw new Error(
      `Failed to create plan section: ${createError?.message ?? "Unknown error"}`,
    );
  }
