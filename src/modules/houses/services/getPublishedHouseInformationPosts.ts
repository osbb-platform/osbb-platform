...
export const getPublishedHouseInformationPosts = cache(
  async (houseId: string): Promise<HouseInformationPostSnapshot[]> => {
    ...
    if (postsError) {
      throw new Error(`Failed to load published information posts: ${postsError.message}`);
    }
    ...
    if (filesError) {
      throw new Error(`Failed to load published information post files: ${filesError.message}`);
    }
