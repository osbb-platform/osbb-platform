...
function SlugIcon() {
  return (
    <svg
      viewBox="0 0 24 24"
      className="h-3 w-3 flex-none text-current"
      fill="none"
      stroke="currentColor"
      strokeWidth={2}
      strokeLinecap="round"
...
const BASE =
  "inline-flex h-8 max-w-full items-center gap-1.5 rounded-[var(--r-pill)] px-3 text-xs leading-none shadow-[var(--cms-shadow-sm)]";
    ...
    <span title={title} className={cx(BASE, VARIANT[variant], className)}>
      ...
      {copyable ? (
        <button
          ...
          onClick={onCopy}
          className="ml-0.5 flex h-3.5 w-3.5 flex-none items-center justify-center rounded-full text-[var(--cms-text-soft)] transition-colors hover:bg-[var(--cms-border-strong)] hover:text-[var(--cms-text)] focus-visible:outline-none focus-visible:shadow-[0_0_0_2px_color-mix(in_srgb,var(--cms-ring)_35%,transparent)]"
        >
          <svg
            viewBox="0 0 24 24"
