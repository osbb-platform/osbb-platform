...
export const Input = React.forwardRef<HTMLInputElement, InputProps>(function Input(
  ...
  return (
    <div className="relative flex items-center">
      {iconLeft && (
        <span className="pointer-events-none absolute left-3.5 text-[var(--cms-text-soft)]" aria-hidden="true">
          {iconLeft}
        </span>
      )}
      <input
        ...
        className={cx(
          "h-11 w-full text-sm rounded-[var(--r-lg)] transition-colors",
        ...
        {...rest}
      />
      {suffix && <span className="absolute right-3.5 text-[var(--cms-text-soft)]">{suffix}</span>}
    </div>
  );
});
