...
describe("P05 T5.2b-2 atomic automation commands",()=>{
 ...
 it("journals manual transition atomically",()=>{ expect(migration).toContain("insert into public.house_plan_status_transitions"); expect(migration).toContain("'manual'"); expect(migration).toContain("auth.uid()"); expect(transition).toContain("resetPlanAutomationInterval"); });
